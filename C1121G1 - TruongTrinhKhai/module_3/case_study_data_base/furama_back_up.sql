create database furama_back_up;
drop database furama_back_up;
use furama_back_up;
...
delete from hop_dong_chi_tiet where hop_dong_chi_tiet.ma_hop_dong in (select ma_hop_dong from hop_dong where hop_dong.ma_khach_hang in (select ma_khach_hang from temp_khach_hang));
select * from temp_khach_hang;
drop table temp_khach_hang;
...
-- Error Code: 1451. Cannot delete or update a parent row: a foreign key constraint fails (`furama_back_up`.`hop_dong`, CONSTRAINT `hop_dong_ibfk_2` FOREIGN KEY (`ma_khach_hang`) REFERENCES `khach_hang` (`ma_khach_hang`))
